...
const statusLabel = { "for-sale": "For Sale", "under-offer": "Under Offer", sold: "Sold" };
...
const PropertyDetail = () => {
  ...
  if (loading) return <div className="container-page py-24 text-sm text-navy-700/60">Loading...</div>;

  if (notFound || !property) {
    return (
      <div className="container-page py-24 text-center">
        <h1 className="font-display text-2xl mb-4">Property not found</h1>
        <Link to="/properties" className="text-brass-600 underline underline-offset-4">Back to properties</Link>
      </div>
    );
  }
  ...
  return (
    <div className="container-page py-12">
      <Link to="/properties" className="text-sm text-navy-700/60 hover:text-brass-600">&larr; Back to properties</Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mt-6">
        <div className="lg:col-span-2">
          ...
          <div className="mt-10">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              ...
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill={isSaved(property.slug) ? "#B8925A" : "none"} stroke={isSaved(property.slug) ? "#B8925A" : "#12233D"} strokeWidth="2">
            ...
            <h1 className="font-display text-3xl md:text-4xl text-navy-900 mb-2">{property.title}</h1>
            <p className="text-navy-700/70 mb-6">
              {property.location?.addressLine ? `${property.location.addressLine}, ` : ""}
              {property.location?.city}, {property.location?.country}
            </p>

            <div className="flex gap-8 border-y border-navy-900/10 py-5 mb-8 text-sm">
              <div><span className="block text-navy-900 font-display text-lg">{property.bedrooms}</span>Bedrooms</div>
              <div><span className="block text-navy-900 font-display text-lg">{property.bathrooms}</span>Bathrooms</div>
              <div><span className="block text-navy-900 font-display text-lg">{property.areaSqft?.toLocaleString()}</span>Sqft</div>
              <div><span className="block text-navy-900 font-display text-lg capitalize">{property.propertyType}</span>Type</div>
            </div>

            <p className="text-navy-800/80 leading-relaxed whitespace-pre-line">{property.description}</p>
          </div>
        </div>
        ...
        <div>
          <div className="sticky top-28 bg-white border border-navy-900/10 p-6">
            ...
            <p className="font-display text-lg text-navy-900 mb-4">Interested in this property?</p>
            <ContactForm
            ...
            <div className="mt-6 pt-6 border-t border-navy-900/10 text-xs text-navy-700/60 space-y-1">
              <p className="text-navy-900 font-medium text-sm mb-1">Prefer to talk directly?</p>
